...
export default async function (msg) {
  ...
  if(contents.status != 'ok')return{error: true, text: '*ERROR*\nsilahkan coba lagi!'}
  ...
  if(contents.link.length  <= 1) {
    ...
    let thumb;
    if(validateThumb) thumb = await msg.resize('./system/image/Error_Thumbnail.png',size)
    else {
  ...
  const contentMedia = await msg.reply(msg.room_chat, {
...
async function setMedia(data, dwd) {
  ...
  for(const content of data) {
    ...
    else if(content.type == 'webp' || content.type == 'jpg' || content.type == "heic") {
